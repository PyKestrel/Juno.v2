...
const commands = [
    ...
    new SlashCommandBuilder().setName('play').setDescription('Search For Music, Play Songs/Playlists From YouTube or Spotify!')
    .addSubcommand((option) =>
        option.setName('search').setDescription('Search For A Song Name').addStringOption(option =>
			option
				.setName('query')
    ...
    .addSubcommand((subcommand) =>
        subcommand.setName('url').setDescription('Pass a YouTube/Spotify URL').addStringOption(option =>
			option
				.setName('link')
				.setDescription('https://www.youtube.com/watch?v=dQw4w9WgXcQ')
                .setRequired(true)
...
client.on('interactionCreate', async interaction => {
    ...
    switch (commandName) {
        ...
        case 'play':
            ...
            if (type == "search") {
                interaction.reply(`Searching For ${interaction.options.data[0].options[0].value}`);
